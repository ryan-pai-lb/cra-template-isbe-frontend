...
export const loader = async({store, request, params}:{store:EnhancedStore, request:Request, params:Params}) => {
...
const TodoList = () => {
  ...
  const [deleteTodo, { isSuccess: isDeleteTodoSuccess, isLoading: isDeleteTodoLoading}] = todosApi.useDeleteTodoMutation();
              ...
              .map((item:TodoItem) => {
                return (
                  <TableRow key={item._uuid} hover role="checkbox" tabIndex={-1} >
                    ...
                    <TableCell><Box color={item.completed ? 'success.main' : 'warning.main'}>{item.completed ? 'Done' : 'Undone'}</Box></TableCell>
                    <TableCell>{ moment.tz(moment.unix(item._modified), 'Asia/Taipei').format('YYYY/MM/DD HH:mm')}</TableCell>
                    <TableCell>
                      <Box className="flex">
                        <IconButton aria-label="delete" size="small" onClick={() => dispatch(globalActions.toogleDialog({
                          visible: true, 
                          close: true,
                          backdropClose: true,
                          confirm: true,
                          contentComponent: <TodoEdit data={item} updateCompleteHandle={() => {
                            dispatch(globalActions.toogleDialog({visible: false}))
                          }}/>,
                          title: `Edit`
                        }))}>
                        ...
                        <IconButton aria-label="delete" size="small" onClick={() => dispatch(globalActions.toogleDialog({
                          ...
                          confirmHandle: () => {
                            deleteTodo([{_uuid: item._uuid}]);
                            dispatch(globalActions.toogleDialog({loading: true}))
                          }
                        }))}>
