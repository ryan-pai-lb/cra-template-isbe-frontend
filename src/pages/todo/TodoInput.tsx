...
const TodoInput = memo((props:TodoInputProps) => {
  ...
  const [updateTodo, {isLoading:isUpdateTodoLoading, isSuccess: isUpdateTodoSuccess}] = todosApi.useUpdateTodoMutation();
  ...
  return (
    <Box p={3} display="flex">
      <Box><FormControlLabel control={<Checkbox checked={completed} onChange={(event:React.ChangeEvent<HTMLInputElement>) => setCompleted(event.target.checked) } />} label="Completed" /></Box>
      <Box flex={1} mr={1}>
        <TextField fullWidth value={message} onInput={(event:React.ChangeEvent<HTMLInputElement>) => setMessage(event.target.value) }/>
      </Box>
